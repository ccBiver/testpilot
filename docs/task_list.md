...
## M1 — 平台化 ✅(2026-07-14 ~ 2026-07-16 完成)
...
- [x] apps/web 管理后台:平台统计、用户管理(禁用/启用,守卫查库即刻生效)、
      全局运行监控;5 个 e2e 测试(2026-07-16)
- [x] 商业化模式重构(2026-07-16 用户决策):撤销用户 BYOK → AI 统一用平台模型
      (管理后台配置,Key 加密);Runner 改为管理员按用户开通(全链路门控);
      注册开关;平台化 8 项 e2e
- [x] 额度体系(2026-07-16 用户决策):用户界面只剩「AI 探索」,启发式转为内部
      冒烟能力;新用户送免费额度(后台可调默认值),平台执行每次扣 1、原子扣减、
      用完 403;Runner(CLI)执行不耗额度;管理员可按用户调额;额度 4 项 e2e
...
- [x] 管理后台独立成站 apps/admin(5181,深色侧边栏,仅 admin 登录,含近14天趋势图;
      用户站移除全部管理代码)(2026-07-16)
- [x] 运行实时进度:内存缓冲 + 轮询接口(云端 onProgress 直写 / Runner 网关批量回传),
      运行页终端风格实时面板;WS 推送在并发上量后再升级(2026-07-16)
- [x] 密码体系:自助修改密码 + 管理员重置(临时密码单次回显)+ 登录页忘记密码指引;
      SMTP 邮件找回待邮件服务就绪(2026-07-16)
