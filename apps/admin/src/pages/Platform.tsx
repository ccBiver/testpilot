...
import { api, ApiError, type PlatformModel } from '../lib/api';
...
export default function Platform() {
  const [model, setModel] = useState<PlatformModel | null>(null);
  ...
  useEffect(() => {
    api
      ...
      .catch(() => {});
    api.getRegistration().then(setRegistration).catch(() => {});
    api.getQuota().then((n) => setDefaultQuota(String(n))).catch(() => {});
  }, []);
  ...
  return (
    <Shell>
      {() => (
        <div>
          <h1 className="text-2xl font-black">平台设置</h1>
          <p className="mt-1 text-sm text-slate-400">模型供给、注册与额度策略</p>
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_300px]">
            ...
            >
              ...
              <p className="mt-1 text-xs text-slate-400">
                全平台用户的 AI 探索统一由这里的模型供能,费用走平台账号。
              </p>
            ...
            >
              ...
              <p className="mt-1 text-xs leading-relaxed text-slate-400">关闭后新用户无法注册,已有用户不受影响。</p>
              <button
                type="button"
              ...
              <div className="mt-6 border-t border-slate-100 pt-5">
                ...
                <p className="mt-1 text-xs leading-relaxed text-slate-400">注册即赠送的 AI 探索次数,仅对新注册用户生效。</p>
